...
function parseArgsWrapper () {
  return {
    parse (inputArgs) {
      ...
      // Normalise boolean values that may have been parsed as strings
      // (e.g. --clean=false becomes "false" string, which is truthy)
      for (const [key, def] of Object.entries(OPTION_DEFS)) {
        if (def.type === 'boolean' && typeof result[key] === 'string') {
          result[key] = result[key] === 'true'
        }
      }

      // --100 shortcut: set all thresholds to 100 and enable check-coverage
      if (result['100']) {
        result.branches = 100
        result.functions = 100
        result.lines = 100
        result.statements = 100
